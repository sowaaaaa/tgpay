...
import requests
import logging
from penalties import register_request, close_request, get_penalties_stats, start_penalty_checker
...
bot = telebot.TeleBot("6795473678:AAGGEVG9HnbQ__CQfrof7DtakplufZoipxQ") #TGPay
# bot = telebot.TeleBot("7416287633:AAGFm8LFpOZtcySjIj7BTAfM9uYiZ3QE0DU")
# bot = telebot.TeleBot("8324268998:AAEpoBlDdLmix5DaR7AFw9YL8O_riD_QRbM") #tgpay test
...
Nicepay = True
adminGroup = -1002090289255
arhiveGroups = [-1002119384184, -1003741046856]
# adminGroup = -5296075748                              #test
# arhiveGroups = [-5296075748, -1003741046856]          #test
...
SEND_TARGET_LIST, CRYPT_SUMM, CRYPT_TXID, RESET_REF_BALANCE, ESIM_MANUAL_SEND, SET_MODERATOR_ID_BALANS, GET_MODERATOR_ID_SUMM= range(69)
...
def _get_system_data(name: str) -> dict:
    try:
        db = sqlite3.connect(USER_DATA_DB)
        cursor = db.cursor()
...
def start_markup(chat_id, text = ""):
    ...
    if text == '🚫 Отмена':
        ...
        markup.add(item4, item12, item5)
        markup.add(item6, types.KeyboardButton('🧮Калькулятор'), item7)
        ...
        if chat_id in admins:
            ...
            markup.add(types.KeyboardButton('🖌Изменить баланс модератору'),
...
def check_check(message):
    ...
    buttons = [
        types.InlineKeyboardButton(text="📣 Подписаться на канал TGPay", url="https://t.me/TGPayNews"),
        types.InlineKeyboardButton(text="📣 Подписаться на чат TGPay", url="https://t.me/+2odRTBF_6jA3YWRi"),
        types.InlineKeyboardButton(text="📣 Подписаться на сообщество VK TGPay", url="https://vk.ru/tgpaygamebot"),
        types.InlineKeyboardButton(text="✅Подписался", callback_data="check_subscribe_button")
    ]
    markup.add(*buttons)
    try:
...
def restart_bot(message):
    if message.chat.id in admins:
        ...
        for moderator in moderators:
            ...
            text = text + f'''\n\n👤 <i>Модератор</i>: @{get_username(moderator)}, <code>{moderator}</code>\n<i>⌞Всего штрафов</i>: <code>{stats['count']}</code>\n<i>⌞Общая сумма</i>: <code>{stats['total']}</code> руб'''
...
def get_phone(message):
    if message.text == "🚫 Отмена":
        ...
        if message.text.isdigit():
            dalshe = False
            if contry == "ua":
                if 5000>=int(message.text)>= 200:
                    # if float(get_balans(message.chat.id))-(float(message.text)*get_kurs('uah'))>=0:
                    dalshe =True
                else:
                    bot.send_message(message.chat.id, "Пополнение возможно от 200 грн до 5000 грн", reply_markup = start_markup(message.chat.id, text='🚫 Отмена'))
                    return
            elif contry == "ru":
...
def send_all(message):
    ...
    if message.text == "🚫 Отмена":
        ...
        action_image = ''
        try:
            action_button = get_par('button_action', message.chat.id)
            print('action_button')
        except:
            print('action_button_false')
        try:
            action_image = get_par('action_image', message.chat.id)
            print('action_image')
        except:
            print('action_image_false')
            pass
            ...
            try:
                ...
                if buttons:
                    for btn in buttons:
                        ...
                        inline_markup.add(types.InlineKeyboardButton(btn["text"], url=btn["url"], style=style))
            except Exception as e:
                print(f"Ошибка кнопок: {e}")

        print(f'action_button={action_button}, action_image={action_image}')
        base_text = message.html_text
        ...
        if action_button == 'True' and action_image != 'True':
            for i, user_id in enumerate(all_users, 1):
                ...
                try:
                    bot.send_message(user_id, get_text_for_user(user_id), reply_markup=inline_markup, parse_mode='HTML')
            ...
            for i, user_id in enumerate(all_users, 1):
                ...
                try:
                    with open(file_names[0], 'rb') as new_file:
                        bot.send_photo(user_id, photo=new_file.read(), caption=get_text_for_user(user_id), parse_mode='HTML')
                        delete_blocked_user(user_id)
                    success_count += 1
                except telebot.apihelper.ApiTelegramException as e:
                    handle_send_error(e, user_id)
            ...
            for i, user_id in enumerate(all_users, 1):
                ...
                try:
                    with open(file_names[0], 'rb') as new_file:
                        bot.send_photo(user_id, photo=new_file.read(), caption=get_text_for_user(user_id), reply_markup=inline_markup, parse_mode='HTML')
                        delete_blocked_user(user_id)
                        success_count += 1
                except telebot.apihelper.ApiTelegramException as e:
                    handle_send_error(e, user_id)
            ...
            for i, user_id in enumerate(all_users, 1):
                print(f'\rРассылка: {i}/{total}', end='', flush=True)
                try:
                    bot.send_message(user_id, get_text_for_user(user_id), parse_mode='HTML')
                    delete_blocked_user(user_id)
                    success_count += 1
                except telebot.apihelper.ApiTelegramException as e:
                    handle_send_error(e, user_id)
        ...
        if errors:
            report += '\n\nОшибки:\n' + '\n'.join(errors)
        bot.send_message(message.chat.id, report, reply_markup = start_markup(message.chat.id))
        delete_file(message.chat.id)
        try:
            for file_name in file_names:
                os.remove(file_name)
...
def set_moderator_id_balans(message):
    if message.text == "🚫 Отмена":
        ...
        if message.text.isdigit():
            ...
            balans = get_moderator_balans(mod_id)
            balans_str = f'{balans} р.' if balans is not None else 'не найден в базе'
            add_data('id_admin', message.text, message.chat.id)
            markup = start_markup(message.chat.id)
            bot.send_message(message.chat.id, f'Баланс модератора {mod_id}: {balans_str}\nВведите сумму для начисления:', reply_markup=markup)
...
def get_moderator_id_summ(message):
    if message.text == "🚫 Отмена":
        ...
        if message.text.replace('-', '').replace('.', '').isdigit():
            markup = start_markup(message.chat.id)
            mod_id = int(get_par('id_admin', message.chat.id))
            success = add_mod_deposit(mod_id, message.text)
            if success:
                new_balans = get_moderator_balans(mod_id)
                bot.send_message(message.chat.id, f'✅ Баланс модератора {mod_id} изменён.\nНовый баланс: {new_balans} р.', reply_markup=markup)
            else:
                bot.send_message(message.chat.id, '❌ Ошибка при изменении баланса.', reply_markup=markup)
            delete_file(message.chat.id)
            update_state(message, START)
        else:
            ...
            if USER_WAIT_FOR_CONTINUE[int(id)]:
                ...
                if float(get_balans(int(id))) >= required:
                    ...
                    if "украина" in service.lower():
                        text = (f"💰 Вы пополнили баланс и теперь можете продолжить оформление заявки:\n\n"
                        f"📞 Телефон: <code>{phone}</code>\n"
                        f"💵 Сумма: <b>{sum_rub}₽</b>\n"
                        f"🇺🇦 В гривнах: <b>{sum_uah}₴</b>\n")
...
def esim_edit(message):
    ...
    if esim_method == "price_tariff":
        ...
        data[tariff_name]["price"] = message.text
...
def _try_deliver_pending_esim(operator):
    ...
    try:
        ...
        image_path = f"eSIM/{esim_entry['image_answer']}.jpg"
        ...
        with open(image_path, "rb") as photo:
            bot.send_photo(user_id, photo, esim_entry["message_answer"], reply_markup=inline_review)
        bot.send_message(
            user_id,
            f'✅ Ваш eSIM готов!\n\n'
...
#ESIM_IMAGE_EDIT
@bot.message_handler(func=lambda message: get_state(message) == ESIM_IMAGE_EDIT, content_types=['photo', 'text'])
def esim_image_edit(message):
    ...
    if esim_method == "auto_tariff":
        ...
        try:
            with open("eSIM/esim_answer.json", encoding="utf-8") as file:
                esim_data = json.load(file)
            if esim_data.get(tariff_name):
                esim_count = int(list(esim_data[tariff_name].keys())[-1]) + 1
            else:
                esim_count = 1
            with open(f"eSIM/{tariff_name}_answer_{esim_count}.jpg", 'wb') as new_file:
                new_file.write(downloaded_file)
            if not esim_data.get(tariff_name):
                esim_data[tariff_name] = {}
            esim_data[tariff_name][esim_count] = {
                "message_answer": message.caption,
                "image_answer": f"{tariff_name}_answer_{esim_count}"
            }
            ...
            try:
                with open("eSIM/esim_answer.json", "w", encoding="utf-8") as file:
                    data = {tariff_name: {"1": {"message_answer": message.caption, "image_answer": f"{tariff_name}_answer_1"}}}
                    json.dump(data, file, ensure_ascii=False, indent=4)
                with open(f"eSIM/{tariff_name}_answer_1.jpg", 'wb') as new_file:
                    new_file.write(downloaded_file)
            except Exception:
                pass
            total = 1
...
def crypt_summ(message):
    ...
    bot.send_message(
        message.chat.id,
        f"Переведите <code>{amount}</code> USDT на адрес (сеть TRC-20):\n\n"
        ...
        try:
            ...
            if 100 <= amount <= 50000:
                ...
                if response.status_code == 200 and "url" in data.get("data", {}):
                    ...
                    # Кнопки "Оплачено" и "Отказаться"
                    inline_markup = types.InlineKeyboardMarkup(row_width=1)
                    inline_markup.add(
                        types.InlineKeyboardButton("✅ Оплачено", callback_data=f"lava_paid:{order_id}"),
                        types.InlineKeyboardButton("❌ Отказаться от оплаты", callback_data="deposit_cancel")
                    )
                ...
                bot.send_message(message.chat.id, "Пополнение LavaPay доступно от 100 до 50000₽",
...
def check_lava_payment(call):
    ...
    if response.status_code == 200 and "status" in data["data"]:
        ...
        if status == "success":
            ...
            add_deposit(call.message.chat.id, amount)
            date = datetime.now().date().strftime('%d.%m.%Y')
...
def handle_callback_query(call):
    ...
    if text == "ua" or text == 'ru' or text == "es":
        ...
        if text == 'my_phone':
            if contry == "ua":
                bot.send_message(chat_id, 'Введите сумму пополнения в гривнах от 200 до 5000 гривен', reply_markup=markup)
            elif contry == 'ru':
        ...
        eSIM_name = call.message.text.split()[-1]
        if text == "auto_tariff":
            bot.send_message(call.message.chat.id, f"Введите и отправьте изображение для автовыдачи", reply_markup=start_markup(chat_id, text='🚫 Отмена'))
...
Для продолжения работы оформите подписку или приобретите новый тариф 💳""")
        ...
        if data.get(operator):
            for index in data[operator].keys():
                os.remove(f"eSIM/{operator}_answer_{index}.jpg")
            del data[operator]
            with open("eSIM/esim_answer.json", "w", encoding="utf-8") as file:
                json.dump(data, file, ensure_ascii=False, indent=4)
        ...
        if text == "Payeer":
            ...
            bot.send_message(call.message.chat.id, 'Введите сумму пополнения от 100₽ до 50 000₽', reply_markup=start_markup(call.message.chat.id, text = '🚫 Отмена'))
            ...
            with open("analytic_clicks_data.json", "w", encoding="utf-8") as json_file:
                json.dump(json_data, json_file, ensure_ascii=False, indent=4)
            bot.send_message(call.message.chat.id, "Введите сумму платежа от 100₽ до 50 000₽", reply_markup = start_markup(call.message.chat.id, text='🚫 Отмена'))
        ...
        tarif = get_par("EsimTarif", chat_id)
        summa = get_par("EsimPrice", chat_id)
        with open("eSIM/esim_answer.json", encoding="utf-8") as file:
            esim_data = json.load(file)
        if esim_data.get(operator):
            if float(get_balans(call.message.chat.id))-float(summa)>=0:
                with open("esim.json", encoding="utf-8") as file:
                    data = json.load(file)
                # esim_buy_count = json.load(open("esim_buy_count.json", encoding="utf-8"))
                # Проверка или создание нового счётчика для пользователя
                user_id = str(call.message.chat.id)  # Ключи должны быть строками в JSON
                # if user_id in esim_buy_count:
                #     if (esim_buy_count[user_id] + 1) % 3 == 0:
                #         bot.send_message(call.message.chat.id, 'Вы можете получить бесплатно 1 eSIM карту, напишите для этого администратору @TGPaySupport_bot', reply_markup=start_markup(call.message.chat.id))
                #     esim_buy_count[user_id] += 1
                # else:
                #     esim_buy_count[user_id] = 1
                # # Сохранение обновлённых данных в JSON
                # with open("esim_buy_count.json", "w", encoding="utf-8") as file:
                #     json.dump(esim_buy_count, file, ensure_ascii=False, indent=4)
                user_type_before = get_user_type(call.message.chat.id)
                add_data('sum', summa, call.message.chat.id)
                update_balanse(call.message.chat.id, 'sum')
                referer = get_ref_user(call.message.chat.id)
                esim_ref_earned = 0
                if referer:
                    usluga = "eSIM"
                    esim_ref_earned = add_balance_ref_with_type(call.message.chat.id, float(summa), usluga, user_type_before)
                    bot.send_message(referer, f'<b>🎉 Вы получили <code>{esim_ref_earned}</code> руб за реферала!</b>\n id:<code>{call.message.chat.id}</code>\n Товар: eSIM', parse_mode="HTML")
                    # bot.send_message(referer, f'Вам пришло {add_balance_ref(id, float(summa), 'eSIM')}Р от реферальной системы')
                update_total_spent(call.message.chat.id, float(summa))
                usluga = f'Esim. {operator}'
                esim_count = list(esim_data[operator].keys())[0]
                image = f"eSIM/{esim_data[operator][esim_count]['image_answer']}.jpg"
                number = to_arhiv(call.message.chat.id, usluga, summa)
                inline_markup2 = types.InlineKeyboardMarkup(row_width=True)

                inline_markup2.add(types.InlineKeyboardButton(text="⬆️ Оставить отзыв", callback_data='send_feedback'))
                with open(image, "rb") as photo:
                    bot.send_photo(call.message.chat.id, photo, esim_data[operator][esim_count]["message_answer"], reply_markup = start_markup(chat_id))
                    bot.send_message(call.message.chat.id, """
Спасибо за покупку eSIM! 🌍
Как только подключите — оставьте отзыв, это помогает нам развиваться 💪                   
                    """, reply_markup=inline_markup2)
                with open(image, "rb") as photo:
                    try:
                        cost_uah = data[operator].get("cost_uah", 0)
                        cost_rate = _uah_cost_rate_cache or get_kurs("uah")
                        cost_rub = round(float(cost_uah) * float(cost_rate), 0)
                        profit = round(float(summa) - cost_rub, 0)
                        net_profit = round(profit - float(esim_ref_earned), 0)
                        ref_block = (
                            f'\n👥 Реферал: -{esim_ref_earned} ₽ → Чистыми: {int(net_profit)} ₽'
                        ) if esim_ref_earned else ''
                        esim_profit_block = (
                            f'\n\n💰 Продажа: {summa} ₽'
                        ...
                        esim_profit_block = f'\n\n💰 Продажа: {summa} ₽'
                    send_photo_to_archives(photo.read(), caption=f'Заявка №{number}\nПользователь: @{call.message.chat.username} \nid: {call.message.chat.id}\n\nУслуга: Esim {operator}\n🎩Ранг: {get_user_rank(call.message.chat.id)}{esim_profit_block}')
                os.remove(image)
                with open("eSIM/esim_answer.json", "w", encoding="utf-8") as file:
                    del esim_data[operator][esim_count]
                    json.dump(esim_data, file, ensure_ascii=False, indent=4)
                ref_user = get_ref_user(call.message.chat.id)
                update_state(call.message, START)
            else:
                update_state(call.message, START)
                bot.send_message(call.message.chat.id, 'Недостаточно средств', reply_markup = start_markup(call.message.chat.id))
            ...
            # Автовыдачи нет — принять оплату и создать заявку для ручной выдачи
            if float(get_balans(call.message.chat.id)) - float(summa) >= 0:
                with open("esim.json", encoding="utf-8") as file:
                    data = json.load(file)
                user_type_before = get_user_type(call.message.chat.id)
                ...
                if referer:
                    usluga_ref = "eSIM"
                    esim_ref_earned = add_balance_ref_with_type(call.message.chat.id, float(summa), usluga_ref, user_type_before)
                    bot.send_message(referer, f'<b>🎉 Вы получили <code>{esim_ref_earned}</code> руб за реферала!</b>\n id:<code>{call.message.chat.id}</code>\n Товар: eSIM', parse_mode="HTML")
                update_total_spent(call.message.chat.id, float(summa))
                usluga = f'Esim. {operator}'
                number = to_arhiv(call.message.chat.id, usluga, summa)
                ...
                bot.send_message(
                    ...
                    f'Услуга: {usluga}\n\n'
                    f'⌚️ Выдача eSIM будет произведена в течение 24 часов.\n'
                    f'При возникновении вопросов обращайтесь к @TGPaySupport_bot',
                    reply_markup=start_markup(chat_id),
                    parse_mode="HTML"
                )
                # Архив с расчётом прибыли
                try:
                    cost_uah = data[operator].get("cost_uah", 0)
                    cost_rate = _uah_cost_rate_cache or get_kurs("uah")
                    cost_rub = round(float(cost_uah) * float(cost_rate), 0)
                    profit = round(float(summa) - cost_rub, 0)
                    net_profit = round(profit - float(esim_ref_earned), 0)
                    ref_block = f'\n👥 Реферал: -{esim_ref_earned} ₽ → Чистыми: {int(net_profit)} ₽' if esim_ref_earned else ''
                    profit_block = (
                        f'\n\n💰 Продажа: {summa} ₽'
                        f'\n💸 Себестоимость: {cost_uah} ₴'
                        f'\n💱 Курс: {cost_rate}'
                ...
                send_to_archives(
                    bot.send_message,
                    f'Заявка №{number}\nПользователь: @{call.message.chat.username}\nid: {call.message.chat.id}\n\n'
                    f'Услуга: Esim {operator}\n🎩Ранг: {get_user_rank(call.message.chat.id)}{profit_block}'
                )
                ...
                with open("eSIM/esim_pending.json", "w", encoding="utf-8") as pf:
                    json.dump(pending_data, pf, ensure_ascii=False, indent=4)
                # Уведомление в группу админов о ручной выдаче
                esim_manual_markup = types.InlineKeyboardMarkup(row_width=True)
                esim_manual_markup.add(
                    types.InlineKeyboardButton("✅ Принять", callback_data=f"esim_manual_accept:{number}:{call.message.chat.id}:{operator}"),
                    types.InlineKeyboardButton("❌ Отклонить", callback_data=f"esim_cancel_ask:{number}:{call.message.chat.id}:{summa}")
                )
                bot.send_message(
                    adminGroup,
                    f'📦 ВЫДАТЬ eSIM ВРУЧНУЮ\n\n'
                    f'Заявка №{number}\n'
                    f'Пользователь: @{call.message.chat.username}\n'
                    f'id: {call.message.chat.id}\n'
                    f'Оператор: {operator}\n'
                    f'🎩Ранг: {get_user_rank(call.message.chat.id)}\n'
                    f'Сумма: {summa} ₽',
        ...
        inline_markup = types.InlineKeyboardMarkup(row_width=True)
        inline_markup.add(types.InlineKeyboardButton("🔴Vodafone", callback_data="Vodafone"))
        inline_markup.add(types.InlineKeyboardButton("🔵Киевстар", callback_data="Kievstar"))
        inline_markup.add(types.InlineKeyboardButton("🟡Lifecell", callback_data="Lifecell"))
    ...
    if 'feed_' in text:
        ...
        inline_markup.add(types.InlineKeyboardButton("🇷🇺Карты РФ", callback_data="BankRF"))
        inline_markup.add(types.InlineKeyboardButton("СБП", callback_data="sbp"))
        inline_markup.add(types.InlineKeyboardButton("🅿️Payeer Rub", callback_data="Payeer"))
            ...
            if text == 'good':
                ...
                try:
                    ...
                    if _cost_rub is not None:
                        add_mod_deposit(call.from_user.id, -_cost_rub)
                        _mod_balans = get_moderator_balans(call.from_user.id) or 0
                        bot.send_message(call.message.chat.id, f'💸 Списано {_cost_rub}₽\n💰 Остаток баланса: {_mod_balans}₽')
                except Exception:
                    pass
                inline_markup = types.InlineKeyboardMarkup(row_width=True)
                inline_markup.add(types.InlineKeyboardButton(text = "⬆️ Оставить отзыв", callback_data = 'send_feedback'))
                ...
                try:
                    ...
                    if "Украина" in usluga:
                        ...
                        try:
                            uah_rate = _uah_cost_rate_cache or get_kurs("uah")
        ...
        if mess_text == '':
            ...
            bot.send_message(call.message.chat.id, mess_text, parse_mode='HTML')
...
def get_text_messages(message):
    ...
    if message.text == '📲Мобильный':
        inline_markup = types.InlineKeyboardMarkup(row_width=True)
        inline_button1 = types.InlineKeyboardButton("🇺🇦Украина", callback_data="ua")
        inline_button2 = types.InlineKeyboardButton("🇷🇺Россия", callback_data="ru")
        inline_button3 = types.InlineKeyboardButton("🇪🇸Испания", callback_data="es")
        inline_markup.add(inline_button1)
        inline_markup.add(inline_button2)
        inline_markup.add(inline_button3)
        bot.send_message(message.chat.id, '🟢 Выберите страну', reply_markup=inline_markup)
        ...
        with open("analytic_clicks_data.json", "w", encoding="utf-8") as json_file:
            json.dump(json_data, json_file, ensure_ascii=False, indent=4)
        bot.send_message(message.chat.id, "✍️По всем вопросам писать\n👨‍💻Администратору @TGPaySupport_bot")
        ...
        mrkp = types.InlineKeyboardMarkup(row_width=True)
        mrkp.add(types.InlineKeyboardButton("📶 Настройка связи", callback_data="private_help"))
        mrkp.add(types.InlineKeyboardButton("💳 Настройка SIM / eSIM", callback_data="private_help"))
        mrkp.add(types.InlineKeyboardButton("📲 Настройка телефона", callback_data="private_help"))
        mrkp.add(types.InlineKeyboardButton("🎮 Настройка игровых аккаунтов", callback_data="private_help"))
        mrkp.add(types.InlineKeyboardButton("🌍 Смена региона аккаунтов", callback_data="private_help"))
        mrkp.add(types.InlineKeyboardButton("💰 Донат в игру", callback_data="private_help"))
        bot.send_message(message.chat.id, "Выберите действие:", reply_markup=mrkp)
        ...
        #markup = types.InlineKeyboardMarkup(row_width=1)
        #markup.add(types.InlineKeyboardButton("🔑 Пробный - 2 дня ", callback_data="trial_vpn"))
        #for name, data in plans.items():
        #    markup.add(
        #       types.InlineKeyboardButton(f"{name} - {data['devices']} устройства", callback_data=f"plan_{name}"))
...
Подключайся и пользуйся интернетом свободно! 🌍💨
        ...
        #bot.send_message(
        #    message.chat.id,
        #    "🚀 Быстрый. Безопасный. Анонимный VPN — за 1 минуту!\n\n"
        #    "🌪 Скорость — как у ветра!\n🔒 Свобода и доступ ко всем сайтам.\n"
        #    "📱 Работает на iOS / Android / Windows / MacOS / Smart TV.\n"
        #    "⚡️ Подключение — в пару кликов.\n🚫 Без рекламы и слежки.\n\n"
        #    "Выбери свой тариф 👇",
        #    reply_markup=markup
        #)
        update_state(message, START)
        ...
        for operator in operators:
            if esim_data.get(operator):
                ...
                count.append(items)
        inline_markup = types.InlineKeyboardMarkup(row_width=True)
        inline_markup.add(types.InlineKeyboardButton(f"🔴Vodafone ({count[0]})", callback_data="admin_Vodafone"))
        inline_markup.add(types.InlineKeyboardButton(f"🔵Киевстар ({count[1]})", callback_data="admin_Kievstar"))
        inline_markup.add(types.InlineKeyboardButton(f"🟡Lifecell ({count[2]})", callback_data="admin_Lifecell"))
